...
func (lb *LoadBalancer) CreateRouter(namespace string, rule string, service string, port int32) error {
	...
	if !strings.Contains(service, "@internal") {
		routerName = service + "_" + strconv.Itoa(int(port))
...
func (lb *LoadBalancer) RemoveRouter(namespace string, rule string, service string, port int32) error {

	mutex.Lock()
	defer mutex.Unlock()

	lbLogger.Info(fmt.Sprintf("Removing router for service [%v] in namespace [%v], with rule [%v] and port [%v]", service, namespace, rule, port))
	err := lb.loadConfig()
	if err != nil {
